      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        ...
        {activeSection === "dashboard" && (
          <div>
            ...
            <div className="relative mb-10 overflow-hidden rounded-3xl border border-emerald-400/20 shadow-2xl">
              <Image
                src="/calcio-totale-dashboard-banner.png"
                alt="Calcio Totale. Una sola passione. Un calciatore con la maglia numero 10 nello stadio."
                width={2048}
                height={682}
                    ...
                    <div className="mt-8 border-t border-slate-800 pt-6">
                      ...
                      <div className="mt-4 overflow-x-auto rounded-2xl border border-slate-800">
                        <table className="min-w-[1000px] w-full text-left text-sm">
                          ...
                          <tbody>{eventReportSummaryRows.length === 0 ? <tr><td colSpan={10 + eventReportHistoryMatches.length} className="px-4 py-7 text-center text-slate-500">Le statistiche appariranno qui dopo il primo salvataggio del referto.</td></tr> : eventReportSummaryRows.map((row) => <tr key={row.player.id} className="border-t border-slate-800"><td className="px-3 py-3 font-bold text-white">{row.player.name}</td>{eventReportHistoryMatches.map((match) => <td key={match.id} className="px-3 py-3 text-center font-bold text-emerald-300">{row.ratingsByMatch.get(match.id) ?? "—"}</td>)}<td className="px-3 py-3 text-center">{row.goals}</td><td className="px-3 py-3 text-center">{row.assists}</td><td className="px-3 py-3 text-center">{row.yellow}</td><td className="px-3 py-3 text-center">{row.red}</td><td className="px-3 py-3 text-center">{row.mvp ? <span className="inline-flex items-center justify-center gap-1 whitespace-nowrap">{row.mvp}<svg viewBox="0 0 24 24" aria-label="Coppa oro MVP" role="img" className="h-4 w-4 text-amber-300" fill="currentColor"><path d="M7 2h10v3h4v3a5 5 0 0 1-5 5h-.03A5 5 0 0 1 13 15.9V19h4v3H7v-3h4v-3.1A5 5 0 0 1 8.03 13H8a5 5 0 0 1-5-5V5h4V2Zm0 5H5v1a3 3 0 0 0 2.1 2.86A5 5 0 0 1 7 10V7Zm10 0v3c0 .3-.03.59-.1.86A3 3 0 0 0 19 8V7h-2Z" /></svg></span> : "—"}</td><td className="px-3 py-3 text-center">{row.mvs ? <span className="inline-flex items-center justify-center gap-1 whitespace-nowrap">{row.mvs}<svg viewBox="0 0 24 24" aria-label="Coppa argento MVS" role="img" className="h-4 w-4 text-slate-300" fill="currentColor"><path d="M7 2h10v3h4v3a5 5 0 0 1-5 5h-.03A5 5 0 0 1 13 15.9V19h4v3H7v-3h4v-3.1A5 5 0 0 1 8.03 13H8a5 5 0 0 1-5-5V5h4V2Zm0 5H5v1a3 3 0 0 0 2.1 2.86A5 5 0 0 1 7 10V7Zm10 0v3c0 .3-.03.59-.1.86A3 3 0 0 0 19 8V7h-2Z" /></svg></span> : "—"}</td><td className="px-3 py-3 text-center">{row.appearances}</td><td className="px-3 py-3 text-center font-black text-emerald-300">{row.average ? row.average.toFixed(2) : "—"}</td></tr>)}</tbody>
                        </table>
                      </div>
                    </div>
